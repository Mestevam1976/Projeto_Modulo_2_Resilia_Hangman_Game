...
Digite a ou b: ''')
        ...
        tema = escolhe_tema.lower()
        if tema == 'a' or escolhe_tema == 'A':  # Não usei método lower ou upper por conta da saída
...
def imprime_mensagem_vencedor(palavra_secreta):
    print(formatting.escolher_cor('green', f"Parabéns {lista_jogadores[0].upper()} você ganhou!"))
...
def imprime_mensagem_perdedor(palavra_secreta):
    print()
    print(formatting.escolher_cor('red', f"Puxa vida! {lista_jogadores[0].upper()} você foi enforcado!"))
    print(formatting.escolher_cor('yellow', f'A palavra secreta era {palavra_secreta.upper()}'))
...
def jogar():
    ...
    while (not acertou and not enforcou):
        ...
        if (chute in palavra_secreta):
            ...
            erros += 1
            print(letras_acertadas)
            print(formatting.escolher_cor('yellow', 'Ainda faltam acertar {} letras'.format(letras_faltando)))
            print(formatting.escolher_cor('blue', 'Você ainda tem {} tentativas'.format(7-erros)))
            desenha_forca(erros)
...
def seleciona_letras(segredo, palavra_em_branco, numero_de_vidas):  # original seleciona_letras
    ...
    somente_letra = False # Para assegurar a funcionalidade do código caso os usuários digitem números ou caracteres especiais

    while somente_letra == False: 
        letra_advinhada = input("Por favor, informe seu palpite, digite somente uma letra: ")
        
        if letra_advinhada.isalpha() == True: # isalpha() retorna True caso a entrada seja uma string do alfabeto
            sinaliza_vidas = 0  
            
            for i in [i for i, x in enumerate(segredo) if x == letra_advinhada.lower()]: # converte a string para minúscula
                sinaliza_vidas = 1  
                palavra_em_branco[i] = letra_advinhada.lower() # converte a string para minúscula
            ...
            print(formatting.escolher_cor('yellow', 'POR FAVOR!!! DIGITE SOMENTE LETRAS MAIÚSCULAS OU MINÍSCULAS, SEM NÚMEROS OU CARACTERES ESPECIAIS!!!'))
